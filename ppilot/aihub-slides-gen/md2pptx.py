    ...
    for slide in prs.slides:
        ...
        if layoutNumber == sectionSlideLayout:
            ...
            sectionName = "".join(
                ...
                if ((letter.isalnum()) | (letter in "&-+") | (letter in "!/*") | (letter == " "))
            )

            sectionName = sectionName.replace("& ", "&amp; ").replace("\r", " ").replace("\n", " ")

            # section URI's just need to be a GUID wrapped in braces
            xml += '      <p14:section name="' + sectionName + '" id="{' + str(uuid.uuid4()).upper() + '}">\n'
...
def _applyCellBorderStyling(tcPr, linePosition, lineWidthMultiplier=1, lineCount=1, lineColour="000000"):
    ...
    lnX.attrib.update({"w": str(lineWidth), "cap": "flat", "cmpd": lineCountValue, "algn": "ctr"})
...
def applyCellBorderStyling(cell, cellBorderStyling, lineWidthMultiplier, lineCount, lineColour):
...
def applyTableLineStyling(
    ...
    for rowNumber, row in enumerate(table.rows):
        if rowNumber + 1 in linedRows:
            ...
            for columnNumber, cell in enumerate(row.cells):
                cellStyling[rowNumber][columnNumber] = cellStyling[rowNumber][columnNumber] + "b"

        elif rowNumber in linedRows:
            # Line before this row so above
            for columnNumber, cell in enumerate(row.cells):
                cellStyling[rowNumber][columnNumber] = cellStyling[rowNumber][columnNumber] + "t"
    ...
    for rowNumber, row in enumerate(table.rows):
        for columnNumber, cell in enumerate(row.cells):
            if columnNumber + 1 in linedColumns:
                ...
                cellStyling[rowNumber][columnNumber] = cellStyling[rowNumber][columnNumber] + "r"
...
def handleSpanClass(run, className):
    if className in bgcolors:
        run = setHighlight(run, bgcolors[className])
...
def makeNumberedListItem(p):
    if p._element.getchildren()[0].tag == "{http://schemas.openxmlformats.org/drawingml/2006/main}pPr":
...
def createRunHyperlinkOrTooltip(run, to_slide, tooltipText=""):
    ...
    if run._parent._parent._parent.__class__.__name__ == "_Cell":
        ...
        shape = run._parent._parent._parent._parent._parent._parent._parent._graphic_frame
    else:
        # Ordinary text run
...
def getPictureTooltip(picture):
    if picture.click_action.hyperlink._hlink is not None:
        # There is a tooltip
...
def findBodyShape(slide):
    if len(slide.shapes) > 1:
        return slide.shapes[1]
    elif slide.shapes.title is None:
        return slide.shapes[0]
    else:
        return None
        ...
        if c == "*":
            ...
            if spanState == "Class":
                ...
                if (className in bgcolors) | (className in fgcolors) | (className in emphases):
                    ...
                    print(f"{className} is not defined. Ignoring reference to it in <span> element.")
...
def createProcessingSummarySlide(presentation, rawMetadata):
    ...
    if processingOptions.hideMetadataStyle:
        ...
        for metadataItem in rawMetadata:
            if metadataItem[0].startswith("style.") is False:
    ...
    newTable = slide.shapes.add_table(rows, columns, tableMargin, contentTop, contentWidth, tableHeight).table
...
def handleWhateverGraphicType(GraphicFilename):
    ...
    if ":" in GraphicFilename:
        ...
        if len(GraphicFilename) > 50:
            printableGraphicFilename = GraphicFilename[:25] + "..." + GraphicFilename[-25:]
        else:
        ...
        try:
            tempGraphicFile = tempfile.NamedTemporaryFile(delete=False, suffix=fileExt, dir=tempDir)
        except IOError as _e:
            print(f"Couldn't create temporary file. md2pptx terminating: {_e}")
            exit()
    ...
    if ".svg" in GraphicFilename.lower():
        ...
        if have_cairosvg:
            ...
            try:
                graphicFile = tempfile.NamedTemporaryFile(delete=False, suffix=".PNG", dir=tempDir)
            except IOError as _e:
                print(f"Couldn't create temporary file. md2pptx terminating: {_e}")
                exit()
        ...
        if have_pillow:
            ...
            if processingOptions.getCurrentOption("exportGraphics"):
                try:
                    ...
                    print(f"Copy error for {PNGname}: {_e}")
...
def handleRunPython(slide, renderingRectangle, codeLines, codeType):
    r = runPython.RunPython()

    r.run(slide, renderingRectangle, codeLines, codeType)
...
def createCodeBlock(slideInfo, slide, renderingRectangle, codeBlockNumber):
        ...
        if codeLines[0] == "```":
            ...
            if codeType.startswith("dot"):
                ...
                if have_graphviz is False:
                    ...
                    sys.stderr.write("GraphViz not installed. Rendering as backticks.\n")
            ...
            elif codeType == "run-python":
                # Built-in Funnel Diagram
                handleRunPython(slide, renderingRectangle, codeLines[1:-1], codeType)
    ...
    fill = codeBox.fill
    fill.solid()
    fill.fore_color.rgb = RGBColor.from_string(processingOptions.getCurrentOption("codeBackground"))
    ...
    fixedPitchHeightWidthRatio = processingOptions.getCurrentOption("fixedPitchHeightWidthRatio")

    estimatedWidthVersusCodeboxWidth = p.font.size * codeColumns / codeBox.width / fixedPitchHeightWidthRatio
    ...
    p.font.color.rgb = RGBColor.from_string(processingOptions.getCurrentOption("codeforeground"))

    # Adjust code box height based on lines
    codeBox.height = min(len(codeLines) * Pt(baseTextSize + 5), renderingRectangle.height)
                ...
                for fragmentNumber, fragment in enumerate(spanFragments):
                    if fragmentNumber > 0:
                        ...
                        if spanClassMatch := spanClassRegex.match(fragment):
                            ...
                            if (className in bgcolors) | (className in fgcolors) | (className in emphases):
                                afterClosingAngle = afterSpanTag[afterSpanTag.index(">") + 1 :]
...
def createListBlock(slideInfo, slide, renderingRectangle):
    ...
    if slideInfo.cards == []:
        ...
        if bulletCount > 0:
            ...
            bulletsShape.height = int(renderingRectangle.height * (100 - cardPercent) / 100)
        ...
        if cardLayout == "horizontal":
            ...
            cardWidth = int((renderingRectangle.width - Inches(horizontalCardGap) * (cardCount - 1)) / cardCount)
        else:
        ...
        for c, card in enumerate(slideInfo.cards):
            ...
            if card.graphic != "":
                ...
                if (card.mediaURL is not None) | (card.graphicTitle is not None):
                    ...
                    graphicTitle = "" if card.graphicTitle is None else card.graphicTitle

                    pictureInfos.append((card.graphicShape, mediaURL, graphicTitle))
                ...
                card.mediaShape = slide.shapes.add_movie(
                    card.mediaInfo.source, Inches(0), Inches(0), Inches(0), Inches(0), card.mediaInfo.poster
                )
            ...
            if cardTitleSize > 0:
                ...
                cardTitleShape.text_frame.paragraphs[0].font.size = Pt(pageTitleSize * 2 / 3)
            ...
            if (cardTitleBackgrounds[0] != ("None", "")) & (cardTitlePosition != "inside"):
                # Card title background picked round-robin from array
                cardTitleBackground = cardTitleBackgrounds[c % len(cardTitleBackgrounds)]
    ...
    for c, card in enumerate(slideInfo.cards):
        ...
        if (cardLayout == "horizontal") | (cardGraphicPosition == "after"):
            ...
            cardBodyShape.left = card.left + cardGraphicSizeRaw + 2 * cardGraphicPadding
        ...
        if cardTitleColour != ("None", ""):
            setColour(cardTitleShape.text_frame.paragraphs[0].font.color, cardTitleColour)
        ...
        if (c > 0) & (cardShape == "line"):
            if cardLayout == "horizontal":
                ...
                dividingLine = createLine(
                    ...
                    cardBackgroundShape.left + cardBackgroundShape.width - int(Inches(0.75)),
                    cardBackgroundShape.top - int(Inches(verticalCardGap / 2)),
                    slide.shapes,
                )
        ...
        if (cardGraphicShape is None) & (cardMediaShape is None):
            ...
            if cardGraphicPosition == "before":
                ...
                if cardLayout == "horizontal":
                    ...
                    cardBodyShape.top = card.bodyTop + cardGraphicSizeRaw + 2 * cardGraphicPadding
                else:
                    # Don't leave room above card for graphic
                    cardBodyShape.top = card.bodyTop
                    cardBodyShape.width = cardWidth - 2 * cardGraphicPadding - cardGraphicSizeRaw
            ...
            if cardLayout == "horizontal":
                ...
                cardBodyShape.height = cardBodyHeight - cardGraphicSizeRaw - 2 * cardGraphicPadding
            else:
                cardBodyShape.height = cardBodyHeight
            ...
            if cardGraphicPosition == "before":
                ...
                if cardLayout == "horizontal":
                    cardMediaShape.top = (
                        cardTitleShape.top
                        + cardGraphicPadding
                        + cardTitleShape.height
                        + int((cardGraphicSizeRaw - cardMediaHeight) / 2)
                    )
                else:
                    cardMediaShape.top = cardBodyShape.top + int((cardBodyHeight - cardMediaHeight) / 2)
            else:
                # Graphic after card text
                if cardLayout == "horizontal":
                    cardMediaShape.top = cardBodyShape.height + cardBodyShape.top + cardGraphicPadding
                else:
                    cardMediaShape.top = cardBodyShape.top + int((cardBodyHeight - cardGraphicSizeRaw) / 2)

            # Horizontally position card graphic shape
            if cardLayout == "horizontal":
                cardMediaShape.left = cardTitleShape.left + int((cardTitleShape.width - cardMediaWidth) / 2)
            else:
                if cardGraphicPosition == "before":
                    cardMediaShape.left = card.left + cardGraphicPadding
                else:
                    cardMediaShape.left = card.left + cardBodyShape.width + cardGraphicPadding
...
def createTableBlock(slideInfo, slide, renderingRectangle, tableBlockNumber):
    ...
    if isGraphicsGrid is True:
        ####################################################################
        # Print the media filenames                                        #
        ####################################################################
        if gridColumns == 2:
            # Doing 1- or 2-row side-by-side graphics slide
            reportGraphicFilenames(printableTopLeftGraphicFilename, printableTopRightGraphicFilename)
        else:
        ...
        if topLeftGraphicFilename != "":
            topLeftMediaWidth, topLeftMediaHeight = getGraphicDimensions(topLeftGraphicFilename)
            if topLeftMediaWidth == -1:
                if gridRows == 2:
                    print("Missing top left image file: " + printableTopLeftGraphicFilename)
                else:
            ...
            if topLeftMediaWidth == -1:
                if gridRows == 2:
                    print("Missing top left video file: " + printableTopLeftGraphicFilename)
                else:
        ...
        if gridColumns == 2:
            ...
            if topRightGraphicFilename != "":
                topRightMediaWidth, topRightMediaHeight = getGraphicDimensions(topRightGraphicFilename)
                if topRightMediaWidth == -1:
                    if gridRows == 2:
                        print("Missing top right image file: " + printableTopRightGraphicFilename)
                    else:
                        print("Missing right image file: " + printableTopRightGraphicFilename)
                ...
                if topRightMediaWidth == -1:
                    if gridRows == 2:
                        print("Missing top right video file: " + printableTopRightGraphicFilename)
                    else:
                        print("Missing right video file: " + printableTopRightGraphicFilename)
        ...
        if gridRows == 2:
            ...
            if bottomLeftGraphicFilename != "":
                bottomLeftMediaWidth, bottomLeftMediaHeight = getGraphicDimensions(bottomLeftGraphicFilename)
                if bottomLeftMediaWidth == -1:
                    print("Missing bottom left image file: " + printableBottomLeftGraphicFilename)
                    return slide
                ...
                if bottomLeftMediaWidth == -1:
                    if gridRows == 2:
                        print("Missing bottom left video file: " + printableBottomLeftGraphicFilename)
                    else:
                        print("Missing left video file: " + printableBottomLeftGraphicFilename)
            ...
            if gridColumns == 2:
                ...
                if bottomRightGraphicFilename != "":
                    ...
                    if bottomRightMediaWidth == -1:
                        print("Missing bottom right image file: " + printableBottomRightGraphicFilename)
                    ...
                    if bottomRightMediaWidth == -1:
                        if gridRows == 2:
                            print("Missing bottom right video file: " + printableBottomRightGraphicFilename)
                        else:
                            print("Missing right video file: " + printableBottomRightGraphicFilename)
        ...
        if (topLeftGraphicFilename != "") | (topLeftVideo is not None):
            (
                ...
                topLeftPicHeight,
                usingHeightToScale,
            ) = scalePicture(maxPicWidth, maxPicHeight, topLeftMediaWidth, topLeftMediaHeight)

            if usingHeightToScale:
                # Calculate horizontal start
                if (gridColumns == 2) and ((topRightGraphicFilename != "") | (topRightVideo is not None)):
                    # Align top left media item to the left
                    topLeftPicX = renderingRectangle.left + (midGraphicsSpaceX - marginBase - topLeftPicWidth) / 2
                else:
        ...
        if topLeftGraphicFilename != "":
            ...
            pictureInfos.append((topLeftPicture, topLeftGraphicHref, topLeftGraphicTitle))
        elif topLeftVideo is not None:
            if topLeftVideoType == "Local":
                ...
                tempVideoFile = tempfile.NamedTemporaryFile(delete=False, suffix="mp4", dir=tempDir)
        ...
        if gridColumns == 2:
            ...
            if (topRightGraphicFilename != "") | (topRightVideo is not None):
                (
                    ...
                    topRightPicHeight,
                    usingHeightToScale,
                ) = scalePicture(maxPicWidth, maxPicHeight, topRightMediaWidth, topRightMediaHeight)

                if usingHeightToScale:
                    # Calculate horizontal start
                    topRightPicX = (renderingRectangle.width + midGraphicsSpaceX - topRightPicWidth) / 2
                else:
                    # Calculate horizontal start
                    topRightPicX = (renderingRectangle.width + midGraphicsSpaceX - topRightPicWidth) / 2

                # Calculate vertical start
                topRightPicY = renderingRectangle.top + (maxPicHeight - topRightPicHeight) / 2
            ...
            if topRightGraphicFilename != "":
                ...
                pictureInfos.append((topRightPicture, topRightGraphicHref, topRightGraphicTitle))
        ...
        if gridRows == 2:
            ...
            if (bottomLeftGraphicFilename != "") | (bottomLeftVideo is not None):
                ...
                if usingHeightToScale:
                    # Calculate horizontal start
                    if (gridColumns == 2) & ((bottomRightGraphicFilename != "") | (bottomRightVideo is not None)):
                        bottomLeftPicX = marginBase + (midGraphicsSpaceX - marginBase - bottomLeftPicWidth) / 2
                    else:
                ...
                # Calculate vertical start
                bottomLeftPicY = renderingRectangle.top + (maxPicHeight + bottomLeftPicHeight) / 2
            ...
            if bottomLeftGraphicFilename != "":
                ...
                pictureInfos.append((bottomLeftPicture, bottomLeftGraphicHref, bottomLeftGraphicTitle))
            ...
            if gridColumns == 2:
                ...
                if (bottomRightGraphicFilename != "") | (bottomRightVideo is not None):
                    ...
                    if usingHeightToScale:
                        # Calculate horizontal start
                        bottomRightPicX = (renderingRectangle.width + midGraphicsSpaceX - bottomRightPicWidth) / 2
                    ...
                    # Calculate vertical start
                    bottomRightPicY = renderingRectangle.top + (maxPicHeight + bottomRightPicHeight) / 2
        ...
        if len(tableRows) > 1:
            ...
            if (firstCellSecondRow.startswith("-")) | (firstCellSecondRow.startswith(":-")):
        ...
        print("           --> " + str(rows) + " x " + str(columns) + " table" + tableHeadingBlurb)
...
def createTOCSlide(presentation, slideNumber, titleText, bullets, tocStyle):
    ...
    if tocStyle != "plain":
        if titleText == tocTitle:
            reportSlideTitle(slideNumber, 3, f'Table Of Contents (Style: "{tocStyle}") {titleText}')
    ...
    TOCEntriesPerRow = int((presentation.slide_width - 2 * marginBase) / (width + entryGap))
    ...
    for entry in TOCEntries:
        ...
        if tocStyle == "chevron":
            ...
            if AbsoluteTOCEntryNumber > 1:
                # z =1
                connector = createLine(x - entryGap / 2, y + height / 2, x, y + height / 2, shapes)
...
def createSlide(presentation, slideNumber, slideInfo):
    ...
    if wantFooters:
        ...
        if leftFooterText != "":
            ...
            leftFooter = slide.shapes.add_textbox(leftFooterBoxLeft, footerBoxTop, leftFooterBoxWidth, footerBoxHeight)

            leftFooter.text, wantHyperLink = substituteFooterVariables(leftFooterText, liveFooters)
        ...
        if middleFooterText != "":
            ...
            middleFooter.text, wantHyperLink = substituteFooterVariables(middleFooterText, liveFooters)
        ...
        if rightFooterText != "":
            ...
            rightFooter.text, wantHyperLink = substituteFooterVariables(rightFooterText, liveFooters)
...
def addSlideTransition(slide, transitionType):
    ...
    xml += "    <mc:Choice xmlns:" + choiceNS + '="' + namespaceURL[choiceNS] + '" Requires="' + choiceNS + '">\n'
    xml += "<p:transition " + namespacesFragment(["p", "p14"]) + 'spd="slow" p14:dur="3400">\n'
    ...
    if transitionType in [
        ...
        xml += "        <p:" + transitionType + ' orient="vert" ' + namespacesFragment(["p"]) + "/>\n"
            ...
            if taskNumber > 0:
                ...
                for row in table.rows:
                    ...
                    if cell0Text not in ["Slide", ""]:
                        ...
                        createRunHyperlinkOrTooltip(run, prs.slides[int(cell0Text) - 2 + templateSlideCount], "")
...
def parse_markdown(input_path: str | Path):
    ...
    for line in lines:
        if line.lstrip().startswith("<!-- md2pptx: "):
            ...
            after_metadata_and_html.append(line)
        ...
        elif (line.lstrip()[:1] == "<") & (in_code is False):
    ...
    return metadata_lines, after_metadata_and_html


def convert_md_to_pptx(input_path: str | Path, output_path: str | Path):
    global banner, cards, slideInfo, pictureInfos, notes_text, indirectAnchors, href_runs, processingOptions
    global graphicRegex, clickableGraphicRegex, videoRegex, audioRegex, linkRegex, spanClassRegex, spanStyleRegex
    global slideHrefRegex, indirectReferenceUsageRegex, abbrevDictionary, abbrevRunsDictionary, footnoteRunsDictionary
    global TOCruns, SectionSlides, want_numbers_headings, want_numbers_content, TOCEntries, numbersHeight
    global bgcolors, fgcolors, emphases, footnoteReferences, maxBlocks, name, colour, prs, templateSlideCount, slide

    start_time = time.time()
    banner = "md2pptx Markdown To Powerpoint Converter " + md2pptx_level + " " + md2pptx_date
    bannerUnderline = ""
    ...
    metadata_lines, after_metadata_and_html = parse_markdown(input_path)

    if len(metadata_lines) + len(after_metadata_and_html) == 0:
    ...
    # Footnotes defaults
    processingOptions.setOptionValuesArray([["footnotesTitle", "Footnotes"], ["footnotesPerPage", 20]])
    ...
    for line in metadata_lines:
        ...
        if name == "numbers":
            ...
            if value in ["left", "right", "center", "centre", "l", "r", "c"]:
                ...
                print(f'PageTitleAlign value \'{value}\' unsupported. "left", "right", "centre", or "center" required.')
            ...
            if value in ["chevron", "circle", "plain"]:
                ...
                print(f'TOCStyle value \'{value}\' unsupported. "chevron" or "circle" required.')
            ...
            processingOptions.setOptionValues("cardBorderColour", parseColour(value.strip()))
            ...
            processingOptions.setOptionValues("cardDividerColour", parseColour(value.strip()))
            ...
            if value in ["horizontal", "vertical"]:
                ...
                print(f'CardLayout value \'{value}\' unsupported. "horizontal" or "vertical" required.')

        elif name == "cardshape":
            if value in ["squared", "rounded", "line"]:
                processingOptions.setOptionValues(name, value)
            else:
                print(f'CardShape value \'{value}\' unsupported. "squared", "rounded", or "line" required.')
            ...
            if value in ["before", "after"]:
                ...
                print(f'cardGraphicPosition value \'{value}\' unsupported. "before",or "after" required.')
            ...
            if value in [
                ...
                print(f'{name} value \'{value}\' unsupported. "vertical" or "horizontal" required.')
        ...
        # Following relate to styling and don't use Processing Options class
            ...
            if value in ["before", "after"]:
                ...
                print(f'funnelLabelPosition value \'{value}\' unsupported. "before" or "after" required.')
    ...
    for line in after_metadata_and_html:
        if startswithOneOf(line, ["<pre>", "<code>"]):
            ...
            if line.lstrip()[0] not in r"*#\|0123456789!":
                if (
                    ...
                    linesAfterConcatenation[-1] = linesAfterConcatenation[-1].rstrip() + " " + line.lstrip()
                else:
                    linesAfterConcatenation.append(line)
    ...
    for lineNumber, line in enumerate(linesAfterConcatenation):
        ...
        if inCode or inHTMLCode or inFencedCode:
            ...
            inTitle = False
        if (line == "") & (inCode is True) & (inHTMLCode is False) & (inFencedCode is False):
            inCode = False
            inTitle = False
        ...
        if line[:1] == "-":
            ...
            if DMMatch := dynamicMetadataRegex.match(line):
                ...
                if (metadataKey != "") & (metadataValue != ""):
                    ...
                    if (metadataKey == "pagesubtitlesize") & (metadataValue == "same"):
                        ...
                        processingOptions.dynamicallySetOption(metadataKey, metadataValue, "")
                    ...
                    # Floating point values where metadata key matches directly
                        ...
                        if metadataValue == "default":
                            numbersheight = processingOptions.getDefaultOption("numbersHeight")
                        elif metadataValue == "pres":
                            numbersheight = processingOptions.getPresentationOption("numbersHeight")
                        else:
                            numbersheight = int(Inches(float(metadataValue)))

                        processingOptions.dynamicallySetOption(metadataKey, numbersheight, "int")
                        ...
                        if metadataValue == "default":
                            ...
                            marginbase = processingOptions.getPresentationOption("marginbase")
                        else:
                            marginbase = int(Inches(float(metadataValue)))

                        processingOptions.dynamicallySetOption(metadataKey, marginbase, "int")
                        ...
                        if metadataValue == "default":
                            ...
                            tablemargin = processingOptions.getPresentationOption("tablemargin")
                        else:
                            tablemargin = int(Inches(float(metadataValue)))

                        processingOptions.dynamicallySetOption(metadataKey, tablemargin, "int")
                    ...
                    # Note: Actual value handling prevents using dynamicallySetOption
                        ...
                        processingOptions.dynamicallySetOption(metadataKey, metadataValue.lower(), "")
                        ...
                        processingOptions.dynamicallySetOption("cardColour", valueArray2, "")
                        ...
                        processingOptions.dynamicallySetOption("cardTitleBackground", valueArray2, "")

                    elif metadataKey in ["funnelcolours", "funnelcolors"]:
                        valueArray2 = [parseColour(c.strip()) for c in metadataValue.split(",")]

                        processingOptions.dynamicallySetOption("funnelColours", valueArray2, "")
                    ...
                    # Note: Actual value handling prevents using dynamicallySetOption
                        ...
                        if metadataValue == "default":
                            contentSplit = processingOptions.getDefaultOption("contentSplit")
                        elif metadataValue == "pres":
                            contentSplit = processingOptions.getPresentationOption("contentSplit")
                        ...
                        processingOptions.dynamicallySetOption("contentSplit", contentSplit, "")
        ...
        elif (line.startswith("#")) & (inCode is False):
            if line[:cardLevel] == "#" * cardLevel:
                ...
                if (notes_text != "") & (slide is not None):
                    createSlideNotes(slide, notes_text)
            ...
            bulletLevel = calculateIndentationLevel(match.start(2), processingOptions.getCurrentOption("indentSpaces"))
        ...
        elif (line.startswith("[")) & line.endswith("]") & (lineNumber == lastTableLine + 1):
    ...
    if (inBlock is True) | (inCode is True) | (inTable is True):
        slideInfo = SlideInfo(slideTitle, slideSubtitle, blockType, bullets, tableRows, cards, code, sequence)
        slideNumber, slide, sequence = createSlide(prs, slideNumber, slideInfo)
    ...
    if len(footnoteDefinitions) > 0:
        slideNumber, footnoteSlides = createFootnoteSlides(prs, slideNumber, footnoteDefinitions)
    ...
    if len(abbrevDictionary) > 0:
        glossaryTermsPerPage = processingOptions.getCurrentOption("glossaryTermsPerPage")
        slideNumber, glossarySlides = createGlossarySlides(prs, slideNumber, abbrevDictionary)
        # Fix up internal glossary hyperlinks
        for abbrevNumber, abbreviation in enumerate(sorted(abbrevRunsDictionary.keys())):
            ...
            for run in runs:
                ...
                createRunHyperlinkOrTooltip(run, glossarySlides[glossarySlideNumber], abbrevDictionary[abbreviation])
    ######################################################################################
    #                                                                                    #
        ...
        for sectionNumber, sectionSlide in enumerate(SectionSlides):
            ...
            if sectionArrowsColour != "":
                for buttonShape in buttonShapes:
                    ...
                    buttonShape.fill.fore_color.rgb = RGBColor.from_string(sectionArrowsColour)
    ...
    for href in href_runs.keys():
        run = href_runs[href]
        if href in slideHrefs:
            createRunHyperlinkOrTooltip(run, prs.slides[slideHrefs[href] - 2 + templateSlideCount], "")
        else:
            ...
            print("Slide " + str(prs.slides.index(SlideFromRun(run)) + 1 - templateSlideCount) + f": '{href}'")
    ...
    try:
        ...
        print(f"deleting the first slide got error: {e}")
        pass
    if processingOptions.getCurrentOption("SectionsExpand"):
        createExpandingSections(prs)
    prs.save(str(output_path))
    elapsed_time = time.time() - start_time
    print("\nProcessing complete.\nElapsed time: " + str(int(1000 * elapsed_time) / 1000) + "s")
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        in_file = sys.argv[1]
        out_file = sys.argv[2]
        convert_md_to_pptx(in_file, out_file)
    else:
        print(f"Invalid args: {sys.argv}\n" f"Usage: python {sys.argv[0]} <input_filename> <output_filename>")
