...
class RunPython:
    ...
    def makeChartData(chart_array, seriesIsColumn=True):
        ...
        # Series
        for _, row in enumerate(chart_array[1:]):
            chart_data.add_series(row[0], row[1:])

        return chart_data

    # Helper function to make a chart. The result can be further manipulated
    def makeChart(slide, chart_type, renderingRectangle, chart_data, title=None, legendPosition=None):
    ...
    def makeTable(slide, renderingRectangle, table_array):
